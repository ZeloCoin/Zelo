    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional ZLO outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes ZLO and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::ZLO_TO_ZLO) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a ZLO-to-ZLO transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::ZLO_ONLY;
        ...
        // If ZLO-to-ZLO fails, create a peg-out transaction
